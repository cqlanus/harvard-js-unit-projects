    ...
    $.fn.playerConnect = function(player){
        ...
        player.onTime(function(evt){
                ...
                for (var i = 0; i < transcriptElements.length; i++){
                  var dataStartStr = $(transcriptElements[i]).attr('data-start');
                  var dataStartNum = parseFloat(dataStartStr);
                  var dataEnd = dataStartNum + parseFloat($(transcriptElements[i]).attr('data-dur'));

                  if(time > dataStartNum && time < dataEnd){
        ...
        return this.click(function(evt){
                ...
                var dataStartStr = $(evt.currentTarget).attr('data-start');
                var dataStartNum = parseFloat(dataStartStr);
                player.seek(dataStartNum);
